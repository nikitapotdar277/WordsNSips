...
def confirm_order():
    if session["type"] == "admin":            
        ...
        if session["order_id"] != 0:
            ...
            total = session["service_charge"] #100
            if session["cart"]["cart_total"] > session["service_charge"]: #if 180 > 100 (true)
                total += session["cart"]["cart_total"] - session["service_charge"] # total = 100 + 180 -100 = 180
...
def dashboard():
    ...
    if orders:
        ...
        for order in orders.keys():
            orders[order]["end_time"] = datetime.strptime(orders[order]["end_time"], "%d/%m/%Y %I:%M %p")
    ...
    now = datetime.now(IST)
    now = now.strftime("%d/%m/%Y %I:%M %p")
    now = datetime.strptime(now, "%d/%m/%Y %I:%M %p")          
...
def add_new_order():
    ...
    for order in orders.keys():
        if orders[order]["phone"] == session["phone"]:
            ...
            if total > entry_fee:
                total = total - entry_fee 
            else:
                total = entry_fee
            
            db.child("orders").child(order).child("order").set(db_cart)
            db.child("orders").child(order).child("total").set(total)
